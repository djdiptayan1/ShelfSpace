...
class FolioService {
    ...
    private let apiKey = "AIzaSyD6FrGW47bSt4sqqDRFibeUeOMWQNdREdE"
    private let baseURL =
            "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-04-17"

        //    private let baseURL = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-pro-preview-05-06"
    ...
    func generateBookSuggestions(primaryMood: String, subChoice: String) async throws -> [BookSuggestion] {
        ...
        let rawJsonResponseString = try await sendRequest(prompt: prompt, expectedMimeType: "application/json")
        print("--- Raw response for BookSuggestions ---\n\(rawJsonResponseString)\n------------------------------------")
        let cleanedJsonResponseString = cleanJsonResponseString(rawJsonResponseString)
        print("--- Cleaned response for BookSuggestions ---\n\(cleanedJsonResponseString)\n------------------------------------")

        do {
            let decoder = JSONDecoder()
            guard let jsonData = cleanedJsonResponseString.data(using: .utf8) else {
                print("Error converting cleaned JSON string to Data for BookSuggestions. Cleaned string was: '\(cleanedJsonResponseString)'")
                throw GeminiServiceError.invalidData
            }
            let geminiBookSuggestions = try decoder.decode([GeminiBookSuggestionWithAuthor].self, from: jsonData)
            return geminiBookSuggestions.map {
                BookSuggestion(title: $0.title, author: $0.author, reason: $0.reason)
            }
        } catch {
            print("Initial parsing error for BookSuggestions: \(error). Problematic JSON: '\(cleanedJsonResponseString)'")
...
//    }
    ...
    func processBookInquiry(query: String) async throws -> String {
        let prompt = """
        You are a knowledgeable AI assistant for a library app. A user is asking about a book.
        User query: "\(query)"
        ...
        let rawJsonResponseString = try await sendRequest(prompt: prompt, expectedMimeType: "application/json")
        print("--- Raw response for Chat Inquiry ---\n\(rawJsonResponseString)\n---------------------------------")
        let cleanedJsonResponseString = cleanJsonResponseString(rawJsonResponseString)
        print("--- Cleaned response for Chat Inquiry ---\n\(cleanedJsonResponseString)\n---------------------------------")

        do {
            let decoder = JSONDecoder()
            guard let jsonData = cleanedJsonResponseString.data(using: .utf8) else {
                print("Error converting cleaned JSON string to Data for Chat. Cleaned string was: '\(cleanedJsonResponseString)'")
                throw GeminiServiceError.invalidData
            }
    ...
    private func sendRequest(prompt: String, expectedMimeType: String = "text/plain") async throws -> String {
...
//             ]
        ...
        if !(200...299).contains(httpResponse.statusCode) {
            ...
            throw GeminiServiceError.apiError("Server returned error code \(httpResponse.statusCode). Response: \(responseBodyString.prefix(500))")
        }
        ...
        do {
            ...
            guard let firstCandidate = jsonResponse.candidates?.first,
                  ...
                  let textOutput = firstPart.text else {
            ...
            throw GeminiServiceError.parsingError("Failed to parse Gemini's main response structure: \(decodingError.localizedDescription).")
        } catch {
    ...
    func fallbackBookSuggestions(for primaryMood: String, subChoice: String) -> [BookSuggestion] {
        return [
            BookSuggestion(
                ...
                author: "Matt Haig",
                reason: "A thought-provoking journey about life's choices, fitting a \(primaryMood.lowercased()) mood seeking \(subChoice.lowercased())."
            ),
            BookSuggestion(
                title: "Project Hail Mary (Fallback)",
                author: "Andy Weir",
                reason: "An exciting space adventure for your \(primaryMood.lowercased()) desire for \(subChoice.lowercased())."
            ),
            BookSuggestion(
                title: "The House in the Cerulean Sea (Fallback)",
                author: "T.J. Klune",
                reason: "A heartwarming tale of found family, bringing \(primaryMood.lowercased()) energy via its \(subChoice.lowercased()) theme."
            ),
        ]
    }
}
