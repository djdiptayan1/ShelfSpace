...
struct ProfileView: View {
    ...
    var body: some View {
        ...
        //     loadUserData()
        // }
        .fullScreenCover(isPresented: .constant(appState.shouldShowLogin)) {
            LoginView()
        }
    }
        ...
        Task {
            do {
                ...
                if let currentUser = try await LoginManager.shared.getCurrentUser() {
                    ...
                    throw NSError(domain: "ProfileError", code: 1, userInfo: [NSLocalizedDescriptionKey: "No user found. Please log in again."])
                }
            } catch {
                await MainActor.run {
                    self.isLoading = false
    ...
    private func formatDate(_ dateString: String) -> String {
        do {
            let date = try JSONUtility.shared.decode(Date.self, from: dateString.data(using: .utf8)!)
    ...
    private func logout() {
        Task {
            do {
                ...
                await MainActor.run {
                    // Reset app state and trigger navigation
                    appState.resetState()
                    dismiss()
                }
            } catch {
