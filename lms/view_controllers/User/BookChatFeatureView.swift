...
struct BookChatFeatureView: View {
    ...
    private var chatInputArea: some View {
        HStack(spacing: 12) {
                ...
                .overlay(
                    Capsule().stroke(
                        inputFieldIsFocused
                            ...
                            : Color.accent(for: colorScheme).opacity(0.7), lineWidth: 1)
                )
            ...
            .animation(
                ...
                value: chatInput.isEmpty || isChatLoading)
        }
        .padding(.horizontal)
        .padding(.vertical, 10)
    }
    ...
    private func exampleChip(_ text: String) -> some View {
        Button(action: {
            ...
            Text(text)
                ...
                .overlay(
                    Capsule().stroke(Color.accent(for: colorScheme).opacity(0.2), lineWidth: 0.5)
                )  // Subtle border
    ...
    private func chatBubble(message: ChatMessage) -> some View {
        HStack(alignment: .bottom, spacing: 8) {
            if !message.isUser {
                // AI avatar
                ZStack {
                    Circle().fill(Color.primary(for: colorScheme).opacity(0.15)).frame(
                        width: 32, height: 32)  // Slightly smaller
                    Image(systemName: "apple.intelligence").font(
                ...
                Text(LocalizedStringKey(message.content))  // << --- THIS IS THE KEY FOR MARKDOWN
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 18, style: .continuous)
                            ...
                            .shadow(
                                color: Color.black.opacity(message.isUser ? 0.08 : 0.04),
                                radius: 3, x: 1, y: 2)
                    )
                    .foregroundColor(Color.text(for: colorScheme))
            ...
            .frame(
                ...
                alignment: message.isUser ? .trailing : .leading)  // Slightly narrower max width

            if message.isUser {
                // User avatar (optional, can be enabled if desired)
                ZStack {
                    Circle().fill(Color.secondary(for: colorScheme).opacity(0.2)).frame(
                        width: 32, height: 32)
                    Image(systemName: "person.fill").font(.system(size: 15)).foregroundColor(
                        Color.secondary(for: colorScheme))
                }
                .padding(.bottom, 10)
            } else {
    ...
    private func sendMessage() {
        ...
        Task {
            ...
            do {
                // Use last 6 messages as context (3 turns of conversation)
                let contextMessages =
                    chatMessages.count <= 6 ? Array(chatMessages) : Array(chatMessages.suffix(6))

                // Pass the chat history to maintain context
                let response = try await FolioService.shared.processBookInquiry(
                    query: userQuery,
                ...
                await MainActor.run {
                    let errorMessageText =
                        "I'm sorry, there was an issue processing your request. Please try again. (\(error.localizedDescription.prefix(50))...)"
                    withAnimation(.interpolatingSpring(stiffness: 170, damping: 20)) {
